...
const Projects = () => {
 ...
 useEffect(() => {
  const fetchRepos = async () => {
    try{
      const response = await axios.get("https://api.github.com/users/AK55068/repos?per_page=10&sort=updated");
      setRepos(response.data);
      setLoading(false);

    }catch (err) {
      setError("Failed to load profile data. please try agin later");
      setLoading(false);
    }
  };
  fetchRepos(); 
