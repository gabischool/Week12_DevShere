...
const Projects = () => {
 ...
 useEffect(() =>{
  const fetchGithubRepo = async() =>{
    try{
      setLoading(true)
      const Response = await axios.get('https://api.github.com/users/farahgarow/repos?per_page=10&sort=updated')
      setRepos(Response.data)      
    }
    catch(error){
      setError('Failed to Load gitHub Respository. Please try again')
      console.error('Error Fetching Github Respos',error)
    }
    finally{
      setLoading(false)
    }
